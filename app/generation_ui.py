...
class GenerationMixin:
    ...
    def _generate_overall_outline(self, meta: dict, concept: str = "", chapter_outline: list = None) -> list:
        ...
        context_lines = [
            f"小说类型：{meta['genre']}",
            f"标题：{meta['title']}",
            f"全书总章数：{real_total}章",
            f"当前批次大纲：{meta.get('chapter_count', len(chapter_outline) if chapter_outline else '?')}章",
        ]
        if protagonist:
...
输出格式：[{{"title": "故事主线", "description": "详细描述", "chapter_range": "第1-5000章"}}, ...]"""
        ...
        fallback = [
            {
                "title": "故事主线",
                "description": f"小说《{meta['title']}》：{meta.get('concept', '')[:200]}。主角{protagonist}在阴阳交界处重生，逐步揭开岛屿秘密。",
                "chapter_range": f"第1-{real_total}章",
            },
            {
                "title": "主要冲突",
                "description": f"{protagonist}被阳人和阴人双重排斥，需在夹缝中求生并证明自己。关键事件：{events_text}",
                "chapter_range": f"第1-{real_total * 2 // 3}章",
            },
            {
                "title": "高潮节点",
                "description": f"{protagonist}掌握阴阳之力后，面对岛屿最大秘密的揭示，引发阴阳两域的终极对决",
                "chapter_range": f"第{real_total * 4 // 5 - 10}-{real_total * 4 // 5 + 10}章",
            },
            {
                "title": "结局走向",
                "description": f"{protagonist}打破阴阳隔阂，重塑岛屿秩序，或选择离开这个不属于他的世界",
                "chapter_range": "最后20章",
            },
        ]
        return fallback
...
⚠️ JSON语法：所有字符串用双引号，冒号后无多余空格，最后一项后不加逗号。"""
        ...
        fallback = {
            ...
            "副线": {
                ...
                "key_events": key_events[4:8]
                if len(key_events) >= 8
                else ["揭示第一个秘密", "发现核心线索", "真相大白"],
            },
        }
        return fallback
...
6. 直接输出优化后的内容"""
    ...
    def _style_imitation(self):
            ...
            def analyze_all():
                for name, text in texts:
                    if len(text) > 100:  # 至少100字才分析
                        ...
                        self.root.after(
                            0,
                            lambda s=style: style_list.insert(
                                tk.END, f"{s.get('author', '未知')} - {s.get('unique_features', '')[:30]}..."
                            ),
                        )
        ...
        # 创作提示
        tk.Label(write_frame, text="创作提示:", bg=C["bg_dark"], fg=C["text_primary"]).pack(
        ...
        ttk.Combobox(
            params_frame, textvariable=word_count_var, values=["500", "1000", "2000", "3000", "5000"], width=8
...
如果没有明显的决策点，输出[]。只检测真正影响故事走向的选择。"""
            ...
            response = self.ai_client.chat(
                [{"role": "user", "content": f"第{chapter_num}章:\n{content[:2000]}"}], system=system, max_tokens=1500
            )
            if not response:
                return
    ...
    def _award_chapter_exp(self, chapter_num: int, content: str):
        ...
        try:
            if not self.character_system or not self.ai_client:
                self._log(
                    f"[角色EXP] 跳过: character_system={self.character_system is not None}, ai_client={self.ai_client is not None}"
                )
                return
            if not self.ai_client.is_configured():
                self._log("[角色EXP] 跳过: AI未配置")
                return
            ...
            _diag.chapter_event(
                chapter_num, "EXP_ANALYSIS_START", {"characters": all_chars[:8], "content_len": len(content)}
            )
...
{{"角色名": {{"action": "行为", "exp": 数值, "detail": "简述"}}}}"""
                    ...
                    response = self.ai_client.chat(
                        ...
                        max_tokens=3000,  # 增大token以容纳思考+JSON
                    )

                    # 详细日志
                    self._log(
                        f"[角色EXP] 第{attempt + 1}次响应: type={type(response).__name__}, len={len(response) if response else 0}"
                    )
            ...
            if not response:
                self._log("[角色EXP] 3次调用均失败，跳过本次EXP发放")
                _diag.chapter_event(chapter_num, "EXP_NO_RESPONSE", {"chars": chars_str, "attempts": 3})
                return

            _diag.chapter_event(
                chapter_num, "EXP_AI_RESPONSE", {"response_len": len(response), "response_preview": response[:200]}
            )
            ...
            if logs:
                ...
                _diag.chapter_event(
                    chapter_num,
                    "EXP_AWARDED",
                    {"total_exp": total_exp, "characters_awarded": len(logs), "details": logs[:5]},
                )
    ...
    def _regen_current_chapter(self):
        ...
        def run():
            try:
                ...
                # 更新显示
                self.root.after(
                    0, lambda c=content, n=current_ch, t=chapter_info.get("title", ""): self._display_chapter(n, t, c)
                )
    ...
    def _auto_generate(self):
        ...
        def run():
            try:
                self._log("=== 开始自动创作 ===")
                self._log("[优化] 已启用内存优化模式，每10章自动清理缓存")
                    ...
                    try:
                        ...
                        with self._state_lock:
                            self.outline = self.agent.generate_outline(
                                meta["genre"], meta["title"], outline_count, concept, total_chapters=real_total
                            )
                ...
                for i, chapter_info in enumerate(outline_snapshot):
                    ...
                    try:
                        ...
                        if not chapter_summary or chapter_summary == "待规划":
                            ...
                            try:
                                real_total = meta.get(
                                    "total_chapters", meta.get("chapter_count", len(outline_snapshot))
                                )
                                ...
                                protagonist = meta.get("protagonist", "")
                                protagonist_hint = (
                                    f"\n【重要】主角名为「{protagonist}」，所有章节必须以此角色为主角！"
                                    if protagonist
                                    else ""
                                )
...
禁止"待规划"。每章必须有具体事件，标题必须反映本章核心内容。"""
                        ...
                        for attempt in range(3):
                            try:
                                ...
                                if "401" in err_msg or "authorization" in err_msg:
                                    self._log("[错误] API认证失败 (401)，请检查API Key!")
                                    raise
                                # 超时判断（只匹配明确的超时错误）
                                is_timeout = any(
                                    kw in err_msg
                        ...
                        if generated == 1 or batch_count % 5 == 0 or ch_num == total:
                            self.root.after(
                                0,
                                lambda c=content, n=ch_num, t=chapter_info.get("title", ""): self._display_chapter(
                                    n, t, c
                                ),
                            )
                            self._log(
                                f"第{ch_num}章创作完成 (进度: {generated}/{total - skipped}, 全书: {ch_num}/{display_total})"
                            )
                        else:
                        ...
                        if "401" in str(e) or "Authorization" in str(e):
                            self._log("[错误] API认证失败，停止所有创作。请检查API Key设置。")
                            self._log(
                                f"当前API配置: provider={self.config.get('api_provider')}, "
            ...
            def run():
                try:
                    ...
                    new_outline = self.agent.generate_outline_continuation(
                        meta.get("genre", "玄幻"), meta.get("title", ""), n, context_text, current_count=last_ch
                    )
                    ...
                    for i, ch_info in enumerate(new_outline):
                        ...
                        content = self.agent.generate_chapter(
                            ch_num,
                            ch_info.get("title", f"第{ch_num}章"),
                            ch_info.get("summary", ""),
                            word_count,
                        ...
                        with self._state_lock:
                            self.current_chapter = ch_num
                        self.root.after(
                            0, lambda c=content, n=ch_num, t=ch_info.get("title", ""): self._display_chapter(n, t, c)
                        )
