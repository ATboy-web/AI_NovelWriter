...
# ==== 导入完成后立刻交还全局不变量（2026-09-17）====
# 上面把 `backend/novel-service` 插到了 sys.path 最前，而那里也有一个 `app` 包
# （`backend/novel-service/app/`）。**`import app` 是全进程共享的名字**，
# 于是 `tests/` 里的用例会读到后端的 `app.__version__ == "1.0.0"`：
# CI 上因此让版本一致性门禁变红、`Build EXE & Release` 被跳过、**Release 没能发布**。
# 本模块只需要上面已经绑定的类对象，不需要让整个测试进程都用这个 app，
# 所以这里把仓库根放回 sys.path 最前，并丢弃被劫持的 `app` 缓存。
# 完整背景见 `tests/_app_authority.py`。
_project_root = str(Path(__file__).resolve().parents[2])
if _project_root in sys.path:
    sys.path.remove(_project_root)
sys.path.insert(0, _project_root)

if "novel-service" in str(getattr(sys.modules.get("app"), "__file__", "")):
    for _name in [name for name in list(sys.modules) if name == "app" or name.startswith("app.")]:
...
class TestNovelGenerator:
    ...
    def test_generator_initialization(self):
        """测试生成器初始化"""
        generator = GenericNovelGenerator(NovelType.SCIFI, "科幻", ["未来科技", "太空探索"], "硬科幻风格")
    ...
    def test_generator_metadata(self):
        """测试生成器元数据"""
        generator = GenericNovelGenerator(NovelType.MYSTERY, "悬疑推理", ["犯罪谜题", "逻辑推理"], "注重逻辑推理")
...
class TestGenericNovelGenerator:
    ...
    async def test_generate_chapter_success(self, generator):
        """测试成功生成章节"""
        mock_response = {"success": True, "content": "这是测试章节内容"}
    ...
    async def test_generate_character_success(self, generator):
        ...
        mock_response = {
            "success": True,
            "character_profile": {"basic_info": {"age": 25, "gender": "男"}, "personality": ["勇敢", "聪明"]},
        }
    ...
    async def test_analyze_style_success(self, generator):
        ...
        mock_response = {
            "success": True,
            "analysis_results": {"language_style": "科幻风格", "narrative_perspective": "第三人称"},
        }
...
class TestGeneratorIntegration:
    ...
    async def test_generate_full_novel_flow(self):
        """测试完整小说生成流程"""
        generator = GenericNovelGenerator(NovelType.SCIFI, "科幻", ["未来科技"], "硬科幻风格")
        ...
        with patch.object(generator, "_call_ai_service", new_callable=AsyncMock) as mock_call:
            mock_call.side_effect = [
                # 大纲响应
                {
                    ...
                    "title": "测试小说",
                    "synopsis": "测试简介",
                    "chapters": [{"title": "第1章", "summary": "章节1"}, {"title": "第2章", "summary": "章节2"}],
                    "characters": [{"name": "主角", "role": "主角", "traits": ["勇敢"]}],
                },
                # 人物响应
                {"success": True, "character_profile": {"basic_info": {"age": 25}, "personality": ["勇敢"]}},
                # 章节1响应
                {"success": True, "content": "章节1内容"},
                # 章节2响应
                {"success": True, "content": "章节2内容"},
            ]
