...
**本次发布的定位**：题材从"写死在界面里"变为**可自定义的注册表**；
新增 **MCP**（双向）与**插件中心**两个能力；以"消除静默失败"为主线做稳定性加固。

> 面板 **15 → 18**（新增 插图工坊 · 插件中心 · MCP 服务器）；
> 题材 **15 类固定 → 135 条内置 + 用户任意自定义**；测试 **2295 → 2985**。
...
**验证性测试的判据不能绑到「子进程退出码」（自检用例的假失败）**
- `test_diagnostic_isolation.py` 的自检用例（在子进程里用真实环境跑完整个文件、再对账
  真实日志字节数）**单独跑绿、跑全量套件红**。原因不是代码问题：本沙箱有个
  「批量删除守卫」（按**单次工具调用**计 50 个路径的预算），全量套件早已把预算耗尽，
  而子进程继承了同一调用标识 ⇒ 它在 pytest 收尾清理临时目录时被 `SystemExit(1)` 打断。
  现象很有辨识度：**子进程 stdout 停在 `[100%]` 而没有汇总行**。
- 修法：把断言从「退出码为 0」改为**只看能说明问题的证据** ——
  ① 真实目录字节未变（主断言，未削弱）；② 子进程确实跑完（防空转）；
  ③ stdout 无 `FAILED` 行才算失败。退出码非 0 但无 FAILED 时只**提示**不判失败。
- 补一条**元测试**钉住这条教训：断言该用例源码里不得再出现 `returncode == 0`。
- 通用教训：**"进程退出码"是环境与代码的混合信号**，把它当断言会让测试对无关机制敏感。
  判断"测试是否通过"要看测试框架自己的结论（`FAILED` 行 / 汇总行），不要看退出码。
