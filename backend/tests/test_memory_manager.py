...
class TestMemoryManager:
    ...
    def test_character_management(self, memory_manager):
        """测试角色管理"""
        characters = {"主角": {"personality": "勇敢", "age": 25}, "配角": {"personality": "忠诚", "age": 30}}
    ...
    def test_settings_management(self, memory_manager):
        """测试世界观设定管理"""
        settings = {"world": "魔法世界", "time": "中世纪", "rules": ["魔法存在", "有龙"]}
...
class TestMemoryManagerRAG:
    ...
    def test_build_smart_context(self, memory_manager_with_data):
        ...
        # 添加角色数据
        manager.save_characters({"主角": {"personality": "勇敢", "age": 25}})

        # 添加世界观设定
        manager.save_settings({"world": "魔法世界", "time": "中世纪"})
...
class TestMemoryManagerIntegration:
    ...
    async def test_full_workflow(self, temp_dir):
        ...
        # 1. 保存世界观设定
        manager.save_settings({"world": "魔法世界", "time": "中世纪", "rules": ["魔法存在", "有龙"]})

        # 2. 保存角色信息
        manager.save_characters(
            {"主角": {"personality": "勇敢", "age": 25}, "导师": {"personality": "智慧", "age": 60}}
        )
