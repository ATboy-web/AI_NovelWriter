...
class CharacterProfile:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "revival_chapter": self.revival_chapter,
        }
...
class CharacterSystem:
    ...
    def _load_all(self):
        ...
        if old_file and old_file.exists():
            try:
                ...
                if imported:
                    logger.info(f"[character_system] 已从旧版单文件导入角色「{name}」，原文件归档为 {archived.name}")
                else:
                    logger.warning(
                ...
                logger.warning(
                    f"[character_system] 旧版单文件 {old_file.name} 处理失败，已保留原文件: {type(e).__name__}: {e}"
                )
    ...
    def random_promote_minor(self, chapter: int) -> Optional[str]:
        """随机提升一个无名小卒为关键人物"""
        minors = [
            name for name, char in self.characters.items() if char.category == "无名小卒" and char.status == "存活"
        ]
        if minors:
    ...
    def ai_create_character(self, ai_client, novel_context: str = "", role: str = "主角") -> Dict:
        """通过AI自动创建角色"""
...
属性范围1-50，主角偏强(20-40)，配角偏弱(10-25)。"""
    ...
    def _save_custom(self):
        if self.custom_file:
            with open(self.custom_file, "w", encoding="utf-8") as f:
                json.dump(
                    {"weapons": self.custom_weapons, "skills": self.custom_skills}, f, indent=2, ensure_ascii=False
                )

    def add_custom_weapon(self, name: str, category: str, quality: str, desc: str, attributes: Dict) -> Dict:
        """添加自定义武器"""
        weapon = {
            "name": name,
            "quality": quality,
    ...
    def add_custom_skill(self, name: str, skill_type: str, desc: str, mp_cost: int = 0, **kwargs) -> Dict:
        """添加自定义技能"""
        skill = {"name": name, "type": skill_type, "desc": desc, "mp_cost": mp_cost, "custom": True, **kwargs}
        ...
        return {
            "剑": [
                ...
                {"name": "龙渊剑", "quality": "宝品", "desc": "龙族神兵", "attributes": {"力量": 30, "体质": 10}},
                {
                    ...
                    "quality": "仙品",
                    "desc": "上古神剑",
                    "attributes": {"力量": 50, "智力": 20, "魅力": 15},
                },
                {
                    "name": "诛仙剑",
                    "quality": "神品",
                    "desc": "斩仙灭神",
                    "attributes": {"力量": 100, "智力": 50, "幸运": 30},
                },
            ],
            "刀": [
            ...
            "弓": [
                {"name": "猎弓", "quality": "凡品", "desc": "猎人之弓", "attributes": {"敏捷": 8, "幸运": 3}},
                {
                    ...
                    "quality": "宝品",
                    "desc": "射日神弓",
                    "attributes": {"敏捷": 30, "力量": 15, "幸运": 20},
                },
                {
                    "name": "后羿弓",
                    "quality": "神品",
                    "desc": "射日神器",
                    "attributes": {"敏捷": 80, "力量": 40, "幸运": 50},
                },
            ],
            "法杖": [
            ...
            "法宝": [
                ...
                {"name": "乾坤袋", "quality": "宝品", "desc": "收纳万物", "attributes": {"体质": 20, "幸运": 15}},
                {
                    ...
                    "quality": "仙品",
                    "desc": "镇压万物",
                    "attributes": {"体质": 40, "智力": 30, "精神": 25},
                },
                {
                    "name": "混沌珠",
                    "quality": "超神",
                    "desc": "混沌之力",
                    "attributes": {"力量": 80, "智力": 80, "体质": 80, "精神": 80},
                },
            ],
            "暗器": [
    ...
    def simulate_battle(self, enemy: Dict) -> Dict:
        ...
        while self.character.hp > 0 and enemy_hp > 0 and round_count < 20:
            ...
            enemy_hp -= damage
            battle_log.append(
                f"第{round_count}回合: {self.character.name}造成{damage}伤害{'（暴击！）' if crit else ''}"
            )
