...
class WebDAVProvider(CloudProvider):
    ...
    def connect(self) -> bool:
        try:
            ...
            self.client = httpx.Client(base_url=self.base_url, auth=(self.username, self.password), timeout=30.0)
            # 测试连接
...
class BaiduPanProvider(CloudProvider):
    ...
    def connect(self) -> bool:
        try:
            if not self.access_token:
                return False
            resp = self.client.get(f"{self.API_BASE}/quota", params={"access_token": self.access_token})
    ...
    def upload(self, local_path: Path, remote_path: str) -> bool:
        ...
        try:
            ...
            resp = self.client.post(
                ...
                json={"path": remote_path, "size": file_size, "isdir": 0, "autoinit": 1, "rtype": 3},
            )
    ...
    def download(self, remote_path: str, local_path: Path) -> bool:
        try:
            resp = self.client.get(
                f"{self.API_BASE}/file",
                params={"method": "download", "access_token": self.access_token, "path": remote_path},
            )
            if resp.status_code == 200:
                local_path.parent.mkdir(parents=True, exist_ok=True)
                with open(local_path, "wb") as f:
    ...
    def list_files(self, remote_path: str = "/") -> List[Dict]:
        try:
            resp = self.client.get(
                f"{self.API_BASE}/file",
                params={"method": "list", "access_token": self.access_token, "dir": remote_path},
            )
            if resp.status_code == 200:
                data = resp.json()
...
class QuarkPanProvider(CloudProvider):
    ...
    def connect(self) -> bool:
        try:
            if not self.cookie:
                return False
            resp = self.client.get(f"{self.API_BASE}/quota", headers={"Cookie": self.cookie})
            self.connected = resp.status_code == 200
            return self.connected
        except Exception:
            return False
    ...
    def upload(self, local_path: Path, remote_path: str) -> bool:
        ...
        try:
            ...
            resp = self.client.post(
                ...
                json={"file_name": local_path.name, "size": file_size, "parent": remote_path, "file_type": "file"},
            )
            ...
            resp = self.client.put(
                f"{self.API_BASE}/file/upload/{upload_id}", headers={"Cookie": self.cookie}, content=file_data
            )
    ...
    def download(self, remote_path: str, local_path: Path) -> bool:
        try:
            resp = self.client.get(
                f"{self.API_BASE}/file/download", headers={"Cookie": self.cookie}, params={"file_id": remote_path}
            )
            if resp.status_code == 200:
                local_path.parent.mkdir(parents=True, exist_ok=True)
                with open(local_path, "wb") as f:
    ...
    def delete_file(self, remote_path: str) -> bool:
        try:
            resp = self.client.post(
                f"{self.API_BASE}/file/delete", headers={"Cookie": self.cookie}, json={"filelist": [remote_path]}
            )
            return resp.status_code == 200
        except Exception:
            return False
...
class XunleiPanProvider(CloudProvider):
    ...
    def connect(self) -> bool:
        try:
            if not self.access_token:
                return False
            resp = self.client.get(f"{self.API_BASE}/quota", headers={"Authorization": f"Bearer {self.access_token}"})
            self.connected = resp.status_code == 200
            return self.connected
        except Exception:
            return False
    ...
    def upload(self, local_path: Path, remote_path: str) -> bool:
        try:
            ...
            resp = self.client.post(
                ...
                json={"name": local_path.name, "size": file_size, "parent_id": remote_path},
            )
    ...
    def download(self, remote_path: str, local_path: Path) -> bool:
        try:
            resp = self.client.get(
                f"{self.API_BASE}/file/{remote_path}/download", headers={"Authorization": f"Bearer {self.access_token}"}
            )
            if resp.status_code == 200:
                local_path.parent.mkdir(parents=True, exist_ok=True)
                with open(local_path, "wb") as f:
    ...
    def delete_file(self, remote_path: str) -> bool:
        try:
            resp = self.client.delete(
                f"{self.API_BASE}/file/{remote_path}", headers={"Authorization": f"Bearer {self.access_token}"}
            )
            return resp.status_code == 200
        except Exception:
            return False
    ...
    def connect(self) -> bool:
        try:
            if not self.access_token:
                return False
            resp = self.client.post(
                f"{self.API_BASE}/user/get", headers={"Authorization": f"Bearer {self.access_token}"}
            )
            self.connected = resp.status_code == 200
            return self.connected
        except Exception:
...
class CloudStorageManager:
    ...
    def upload_novel(self, novel_dir: Path, provider_id: str, remote_path: str = "/AI_NovelWriter") -> bool:
        """上传小说到云端"""
        provider = self.providers.get(provider_id)
    ...
    def download_novel(self, remote_path: str, local_dir: Path, provider_id: str) -> bool:
        """从云端下载小说"""
        provider = self.providers.get(provider_id)
        if not provider or not provider.connected:
            return False
    ...
    def sync_novel(self, novel_dir: Path, provider_id: str, remote_path: str = "/AI_NovelWriter") -> bool:
        """同步小说到云端（增量同步）"""
        provider = self.providers.get(provider_id)
        if not provider or not provider.connected:
            return False
